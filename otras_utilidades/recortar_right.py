...
DST_DIR = Path(r"C:\Users\Administrador\Documents\INGENIERIA_EN_SOFTWARE\TESIS\CODIGO\validador_retratos_webrtc\cuts\RESULT") # ← change this
...
def crop_right_excess_inplace(image_path: str | Path) -> Tuple[int, int]:
    """
    If image width > 277, remove (width - 277) columns from the RIGHT edge and overwrite the same file.
    Returns (new_width, new_height). If width <= 277, leaves file unchanged and returns original size.
    """
    ...
    with Image.open(path) as im:
        ...
        cropped = im.crop((0, 0, w - diff, h))  # keep left portion
